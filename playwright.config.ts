...
export default defineConfig({
  ...
  webServer: {
    command: [
      ...
      ` && ${e2eEnv} corepack pnpm build`,
      ` && ${e2eEnv} corepack pnpm wrangler dev --local --persist-to "${persistDir}" --ip ${host} --port ${port} --var YOSAN_FLOW_E2E_RESET_TOKEN:local-e2e-reset-token`,
      "'",
    ].join(""),
